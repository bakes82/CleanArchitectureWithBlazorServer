...
public class ExportLogsQueryHandler : IRequestHandler<ExportLogsQuery, byte[]>
{
    ...
    public ExportLogsQueryHandler(IApplicationDbContext context, IMapper mapper, IExcelService excelService, IStringLocalizer<ExportLogsQueryHandler> localizer)
    {
        _context      = context;
        _mapper       = mapper;
    ...
    {
        List<LogDto> data = await _context.Loggers.Where(x => x.Message!.Contains(request.Keyword) || x.Exception!.Contains(request.Keyword))
                                          .OrderBy($"{request.OrderBy} {request.SortDirection}")
                                          .ProjectTo<LogDto>(_mapper.ConfigurationProvider)
                                          .ToListAsync(cancellationToken);
        byte[] result = await _excelService.ExportAsync(data, new Dictionary<string, Func<LogDto, object?>>
                                                              {

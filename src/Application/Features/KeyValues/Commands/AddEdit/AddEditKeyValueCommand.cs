...
public class AddEditKeyValueCommand : ICacheInvalidatorRequest<Result<int>>
{
    ...
    private class Mapping : Profile
    {
        public Mapping()
        {
            CreateMap<KeyValueDto, AddEditKeyValueCommand>()
                .ReverseMap();
        }
    }
}
...
public class AddEditKeyValueCommandHandler : IRequestHandler<AddEditKeyValueCommand, Result<int>>
{
    ...
    {
        KeyValueDto? dto = _mapper.Map<KeyValueDto>(request);

        if (request.Id > 0)
        {
            KeyValue? keyValue = await _context.KeyValues.FindAsync(new object[]
                                                                    {
                                                                        request.Id
                                                                    }, cancellationToken);
        ...
        {
            KeyValue? keyValue = _mapper.Map<KeyValue>(dto);
            keyValue.AddDomainEvent(new UpdatedEvent<KeyValue>(keyValue));

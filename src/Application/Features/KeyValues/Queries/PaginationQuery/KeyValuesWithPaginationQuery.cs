...
public class KeyValuesQueryHandler : IRequestHandler<KeyValuesWithPaginationQuery, PaginatedData<KeyValueDto>>
{
    ...
    public KeyValuesQueryHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper  = mapper;
    }
    ...
    {
        PaginatedData<KeyValueDto> data = await _context.KeyValues.OrderBy($"{request.OrderBy} {request.SortDirection}")

namespace Application.Example.Features.Documents.Specifications;
...
public class AdvancedDocumentsSpecification : Specification<Document>
{
    public AdvancedDocumentsSpecification(AdvancedDocumentsFilter filter)
    {
        ...
        Query.Where(p => p.CreatedBy == filter.CurrentUser.UserId && p.IsPublic == false || p.IsPublic == true && p.TenantId == filter.CurrentUser.TenantId, filter.ListView == DocumentListView.All)
             .Where(p => p.CreatedBy == filter.CurrentUser.UserId && p.TenantId == filter.CurrentUser.TenantId, filter.ListView                                              == DocumentListView.My)

...
public class AddEditProductCommand : ICacheInvalidatorRequest<Result<int>>
{
    ...
    private class Mapping : Profile
    {
        public Mapping()
        {
            CreateMap<AddEditProductCommand, ProductDto>()
                .ReverseMap();
        }
    }
}
...
public class AddEditProductCommandHandler : IRequestHandler<AddEditProductCommand, Result<int>>
{
    ...
    {
        ProductDto? dto = _mapper.Map<ProductDto>(request);
        if (request.Id > 0)
        {
            Product? item = await _context.Products.SingleOrDefaultAsync(x => x.Id == request.Id, cancellationToken) ?? throw new NotFoundException($"Product with id: {request.Id} not found.");

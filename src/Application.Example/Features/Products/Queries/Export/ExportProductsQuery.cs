...
namespace Application.Example.Features.Products.Queries.Export;
...
public class ExportProductsQueryHandler : IRequestHandler<ExportProductsQuery, Result<byte[]>>
{
    ...
    public ExportProductsQueryHandler(IApplicationDbContext context, IMapper mapper, ISerializer serializer, IExcelService excelService, IPdfService pdfService, IStringLocalizer<ExportProductsQueryHandler> localizer)
    {
        _context      = context;
        _mapper       = mapper;
...
#nullable disable warnings
    ...
    {
        List<ProductDto>? data = await _context.Products.ApplySpecification(request.Specification)
                                               .AsNoTracking()

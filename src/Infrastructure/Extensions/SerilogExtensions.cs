...
public static class SerilogExtensions
{
    ...
    private static void WriteToDatabase(LoggerConfiguration serilogConfig, IConfiguration configuration)
    {
        ...
        string? dbProvider       = configuration.GetValue<string>($"{nameof(DatabaseSettings)}:{nameof(DatabaseSettings.DbProvider)}");
        string? connectionString = configuration.GetValue<string>($"{nameof(DatabaseSettings)}:{nameof(DatabaseSettings.ConnectionString)}");
        switch (dbProvider)
        {
    ...
    {
        PrivacySettings? privacySettings = configuration.GetRequiredSection(PrivacySettings.Key)
                                                        .Get<PrivacySettings>();
        ...
        if (privacySettings.LogClientIpAddresses)
        {
            serilogConfig.Enrich.WithClientIp();
        }

        if (privacySettings.LogClientAgents)
        {
    ...
    private static void WriteToSqlServer(LoggerConfiguration serilogConfig, string? connectionString)
    {
        ...
        serilogConfig.WriteTo.Async(wt => wt.MSSqlServer(connectionString, sinkOpts, columnOptions: columnOpts));
    }
    ...
    private static void WriteToNpgsql(LoggerConfiguration serilogConfig, string? connectionString)
    {
        ...
        serilogConfig.WriteTo.Async(wt => wt.PostgreSQL(connectionString, tableName, columnOptions, LogEventLevel.Information, needAutoCreateTable: false, schemaName: "public", useCopy: false));
    }
    ...
    private static void WriteToSqLite(LoggerConfiguration serilogConfig, string? connectionString)
    {
        ...
        const string tableName = "Loggers";
        serilogConfig.WriteTo.Async(wt => wt.SQLite(connectionString, tableName, LogEventLevel.Information));
    }

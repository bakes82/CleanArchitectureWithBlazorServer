...
public class LocalizationCookiesMiddleware : IMiddleware
{
    ...
    {
        Provider = requestLocalizationOptions.Value.RequestCultureProviders.Where(x => x is CookieRequestCultureProvider)
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        if (Provider != null)
        {
            ...
            if (feature != null)
            {
                ...
                context.Response.Cookies.Append(Provider.CookieName, CookieRequestCultureProvider.MakeCookieValue(feature.RequestCulture), new CookieOptions { Expires = new DateTimeOffset(DateTime.Now.AddMonths(3)) });
            }
        }

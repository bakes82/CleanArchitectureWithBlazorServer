...
public class ResetPasswordFormModelValidator : AbstractValidator<ResetPasswordFormModel>
{
    ...
    public Func<object, string, Task<IEnumerable<string>>> ValidateValue =>
        ...
        {
            ValidationResult? result = await ValidateAsync(ValidationContext<ResetPasswordFormModel>.CreateWithOptions((ResetPasswordFormModel)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
            {

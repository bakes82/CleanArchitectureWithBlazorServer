...
public class ChangePasswordModelValidator : AbstractValidator<ChangePasswordModel>
{
    ...
    public Func<object, string, Task<IEnumerable<string>>> ValidateValue =>
        ...
        {
            ValidationResult? result = await ValidateAsync(ValidationContext<ChangePasswordModel>.CreateWithOptions((ChangePasswordModel)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
            {

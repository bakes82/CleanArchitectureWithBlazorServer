...
public class ApplicationUserDtoValidator : AbstractValidator<ApplicationUserDto>
{
    ...
    public Func<object, string, Task<IEnumerable<string>>> ValidateValue =>
        ...
        {
            ValidationResult? result = await ValidateAsync(ValidationContext<ApplicationUserDto>.CreateWithOptions((ApplicationUserDto)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
            {

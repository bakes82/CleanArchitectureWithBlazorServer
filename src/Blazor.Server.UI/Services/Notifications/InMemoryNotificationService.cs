...
public class InMemoryNotificationService : INotificationService
{
    ...
    public InMemoryNotificationService(ProtectedLocalStorage localStorageService, ILogger<InMemoryNotificationService> logger)
    {
    ...
    public async Task MarkNotificationsAsRead(string id)
    {
        ...
        ProtectedBrowserStorageResult<DateTime> timestamp = await _localStorageService.GetAsync<DateTime>(LocalStorageKey);
    ...
    public async Task<IDictionary<NotificationMessage, bool>> GetNotifications()
    {
        ...
        Dictionary<NotificationMessage, bool> items             = _messages.ToDictionary(x => x, x => lastReadTimestamp > x.PublishDate);
        return items;
    }
    ...
    public void Preload()
    {
        _messages.Add(new NotificationMessage("mudblazor-here-to-stay", "MudBlazor is here to stay", "We are paving the way for the future of Blazor", "Announcement", new DateTime(2022, 01, 13),
                                              "_content/MudBlazor.Docs/images/announcements/mudblazor_heretostay.png", new[]
                                                                                                                       {
                                                                                                                           new NotificationAuthor("Jonny Larsson", "https://avatars.githubusercontent.com/u/10367109?v=4")
